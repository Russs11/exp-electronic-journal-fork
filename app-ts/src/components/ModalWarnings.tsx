    ...
    if (item.getResult() === '' || item.getResult() === 'не указано') {
      ...
      if (item.getDatePetitionStart() === '') {
        ...
        const daysCount = Math.floor(
          (Number(date1) - Number(date2)) / 1000 / 60 / 60 / 24
        )
        ...
        if (daysCount < 0) {
          const dayOfWeek =
            daysCount < -5 ? 'дней' : daysCount === -1 ? 'день' : 'дня'
          const text = `№${item.getId()} - ${
            daysCount * -1
          } ${dayOfWeek} после окончания экспертизы`
          expWarningsComponentsArr.push(
            <WarningsText
              key={item.getId()}
              text={text}
      ...
      if (
        ...
        if (daysCount <= 3 && daysCount >= 0) {
          const dayOfWeek =
            daysCount === 0 ? 'дней' : daysCount === 1 ? 'день' : 'дня'
          const text = `№${item.getId()} - ${daysCount} ${dayOfWeek} до окончания ходатайства`
          petitionsWarningsComponentsArr.push(
            <WarningsText
              key={item.getId()}
              text={text}
              number={item.getId()}
        ...
        if (daysCount < 0) {
          ...
          const text = `№${item.getId()} - ${
            daysCount * -1
          } ${dayOfWeek} после окончания ходатайства`
          petitionsWarningsComponentsArr.push(
            <WarningsText
              key={item.getId()}
              text={text}
